...
export const fetchWards = createAsyncThunk("wards/fetchWards", async () => {
  const response = await axios.get(
    "https://patient-management-one.vercel.app/api/v1/ward",
  );
...
export const addWardAsync = createAsyncThunk(
  ...
  async (ward) => {
    const response = await axios.post(
      "https://patient-management-one.vercel.app/api/v1/ward/new",
      ward,
    );
...
export const updateWardAsync = createAsyncThunk(
  ...
  async ({ id, ward }) => {
    const response = await axios.put(
      `https://patient-management-one.vercel.app/api/v1/ward/update/${id}`,
      ward,
    );
...
export const deleteWardAsync = createAsyncThunk(
  "ward/deleteWardAsync",
  async (id) => {
    const respone = await axios.delete(
      `https://patient-management-one.vercel.app/api/v1/ward/delete/${id}`,
    );
    return id;
  },
);
